...
con=c.connect(host='localhost',user='ROWDY',passwd='mysql',database='hotel')
cursor=con.cursor()
...
def booking():
	...
	while True:
		...
		if n!="" and p1!="" and a!="":
			...
			print("\tName, Phone no. & Email cannot be empty..!!")
...
def restaurant():
	...
	for n in csvr:
		if n[8]==str(ph):
			...
			customerid=(n[8])
			print(' ')
			print('Welcome',n[0],".What would you like to have today?")
			print("----------------------------------------------------------------")
			print("----------------------------------------------------------------") 
...
def payment():
	...
	id=str(id)
	f=open("services.csv","r")
	csvr=csv.reader(f)
	for n in csvr:
		if n[1]==id:
			print("Welcome",n[0],". Here is your bill:")
...
def delete_record():
	...
	st="delete from booking where Customer_id={}".format(id)
	cursor.execute(st)
	con.commit()
